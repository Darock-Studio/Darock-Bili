//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 66.0.0.0.0
//
#ifndef DebugHierarchyRequestObjectAction_Protocol_h
#define DebugHierarchyRequestObjectAction_Protocol_h
@import Foundation;

@protocol DebugHierarchyRequestObjectAction <DebugHierarchyRequestAction>
/* instance methods */
- (void)performInContext:(id)context withObject:(id)object;
- (BOOL)targetsObjectIdentifiers:(id *)identifiers;
@end

#endif /* DebugHierarchyRequestObjectAction_Protocol_h */
