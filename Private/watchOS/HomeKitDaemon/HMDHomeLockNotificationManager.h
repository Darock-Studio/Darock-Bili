//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 1092.3.10.0.0
//
#ifndef HMDHomeLockNotificationManager_h
#define HMDHomeLockNotificationManager_h
@import Foundation;

#include "HMFObject.h"
#include "HMDHome.h"
#include "HMDHomeLockNotificationManagerDataSource-Protocol.h"
#include "HMFLogging-Protocol.h"
#include "HMFMessageReceiver-Protocol.h"

@class HMFMessageDispatcher, NSString, NSUUID;
@protocol OS_dispatch_queue;

@interface HMDHomeLockNotificationManager : HMFObject<HMFMessageReceiver, HMFLogging>

@property (readonly, copy) NSUUID *uuid;
@property (weak) HMDHome *home;
@property (retain) HMFMessageDispatcher *msgDispatcher;
@property (readonly) NSObject<HMDHomeLockNotificationManagerDataSource> *dataSource;
@property (readonly) NSObject<OS_dispatch_queue> *workQueue;
@property (readonly) BOOL hasReachablePrimaryResidentSupportingLockNotificationContext;
@property (readonly, nonatomic) NSUUID *messageTargetUUID;
@property (readonly, nonatomic) NSObject<OS_dispatch_queue> *messageReceiveQueue;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

/* class methods */
+ (id)findEcosystemName:(id)name;
+ (id)logCategory;

/* instance methods */
- (void)handleDoorLockEventReport:(id)report forAccessory:(id)accessory flow:(id)flow;
- (void)handleDoorLockAlarmEvent:(id)event forAccessory:(id)accessory flow:(id)flow;
- (void)handleLockOperationEvent:(id)event forAccessory:(id)accessory flow:(id)flow;
- (void)handleLockUserChangeEvent:(id)event forAccessory:(id)accessory flow:(id)flow;
- (void)sendLockOperationEventNotification:(id)notification lockOperationType:(id)type lockOperationSource:(id)source fabricIndex:(id)index accessory:(id)accessory timestamp:(id)timestamp flow:(id)flow;
- (void)sendDoorLockAlarmEventNotification:(id)notification accessory:(id)accessory flow:(id)flow;
- (void)sendLockUserChangeEventNotification:(id)notification lockDataType:(unsigned char)type accessory:(id)accessory dataOperationType:(unsigned char)type fabricIndex:(id)index flow:(id)flow;
- (id)initWithUUID:(id)uuid workQueue:(id)queue;
- (id)initWithUUID:(id)uuid workQueue:(id)queue dataSource:(id)source;
- (void)configureWithHome:(id)home;
- (BOOL)shouldResolveNotificationContextForCharacteristic:(id)characteristic;
- (void)handleResolveNotificationContextIdentifierMessage:(id)message;
- (void)resolveEncodedCharacteristicNotificationContext:(id)context accessoryUUID:(id)uuid completion:(id /* block */)completion;
- (id)displayNameForNotificationContextResolutionResult:(id)result;
- (void)resolveKeypadContextIdentifier:(id)identifier accessoryUUID:(id)uuid withCompletion:(id /* block */)completion;
- (void)_sendResolveLockNotificationContextIdentifierMessage:(id)message accessoryUUID:(id)uuid withCompletion:(id /* block */)completion;
- (void)_resolveKeypadContextIdentifier:(id)identifier accessoryUUID:(id)uuid completion:(id /* block */)completion;
- (id)contextResolutionResultFromAccessCode:(id)code;
- (id)contextResolutionResultFromHAPContextId:(id)id;
- (id)contextResolutionResultFromNFCContextId:(id)id;
- (id)logIdentifier;
@end

#endif /* HMDHomeLockNotificationManager_h */
