//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 7209.1.301.0.0
//
#ifndef UIStatusBarPublisher_h
#define UIStatusBarPublisher_h
@import Foundation;

@class NSMutableSet;

@interface UIStatusBarPublisher : NSObject {
  /* instance variables */
  struct __CFMachPort * _machPort;
  unsigned long long _styleOverrides;
  NSMutableSet *_statusBarItems;
}

/* instance methods */
- (id)initWithCFMachPort:(struct __CFMachPort *)port;
- (void)dealloc;
- (unsigned int)publisherPort;
- (unsigned long long)styleOverrides;
- (unsigned long long)addStyleOverrides:(unsigned long long)overrides;
- (unsigned long long)removeStyleOverrides:(unsigned long long)overrides;
- (id)statusBarItems;
- (void)addStatusBarItem:(int)item;
- (void)removeStatusBarItem:(int)item;
@end

#endif /* UIStatusBarPublisher_h */
