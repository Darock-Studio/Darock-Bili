//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 7209.1.301.0.0
//
#ifndef _UIActionBridge_h
#define _UIActionBridge_h
@import Foundation;

@interface _UIActionBridge : NSObject
/* class methods */
+ (id)actionMenuForMenu:(id)menu firstTarget:(id)target includeHidden:(BOOL)hidden validation:(id)validation;
+ (id)actionMenuForMenu:(id)menu firstTarget:(id)target includeHidden:(BOOL)hidden;
+ (id)validatedCommandMenuForMenu:(id)menu context:(id)context validation:(id)validation;
+ (id)validatedCommandMenuForMenu:(id)menu context:(id)context;
@end

#endif /* _UIActionBridge_h */
