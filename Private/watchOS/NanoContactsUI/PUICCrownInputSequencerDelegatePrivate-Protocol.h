//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 378.2.12.0.0
//
#ifndef PUICCrownInputSequencerDelegatePrivate_Protocol_h
#define PUICCrownInputSequencerDelegatePrivate_Protocol_h
@import Foundation;

@protocol PUICCrownInputSequencerDelegatePrivate <NSObject>
@optional
/* instance methods */
- (void)_crownInputSequencer:(id)sequencer offsetDidChangeByDelta:(double)delta;
- (BOOL)_crownInputSequencerShouldSuppressInput:(id)input;
- (void)_crownInputSequencer:(id)sequencer offsetDidChangeByDistance:(double)distance;
@end

#endif /* PUICCrownInputSequencerDelegatePrivate_Protocol_h */
