//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 141.1.1.0.0
//
#ifndef _EXExtensionConfigurationProviding_Protocol_h
#define _EXExtensionConfigurationProviding_Protocol_h
@import Foundation;

@protocol _EXExtensionConfigurationProviding <_EXExtensionPointConfigurationProviding>

@property (readonly) unsigned long long type;
@property (readonly) NSString *internalServiceName;
@property (readonly) NSString *serviceName;

@end

#endif /* _EXExtensionConfigurationProviding_Protocol_h */
