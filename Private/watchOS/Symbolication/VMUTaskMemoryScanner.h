...
@class NSDate, NSDictionary, NSMapTable, NSMutableArray, NSMutableDictionary, NSString;
...
- (id)initFullyWithTask:(unsigned int)task;
...
- (BOOL)_callRemoteMallocEnumerators:(unsigned int)enumerators error:(id *)error block:(id /* block */)block;
- (void)callRemoteMallocPrints:(unsigned int)prints printer:(undefined *)printer;
...
- (void)_identifyNonObjectsPointingToSwiftMetadata:(unsigned int)metadata;
- (id)_recursivelyIdentifySwiftAsyncTaskSlabs:(unsigned long long)slabs containerRange:(struct _VMURange { unsigned long long x0; unsigned long long x1; })range remoteBlock:(unsigned long long)block mappedBlock:(unsigned long long)block asyncTaskSlabMetadataIsa:(unsigned long long)isa nestingLevel:(unsigned int)level;
...
- (struct _VMURegionNode { id x0; unsigned int * x1; unsigned int x2; unsigned int x3; unsigned long long x4; unsigned long long x5; unsigned long long x6; void * x7; struct _VMURegionNode * x8; } *)validateAddressRangeForSwiftRemoteMirror:(struct _VMURange { unsigned long long x0; unsigned long long x1; })mirror allowUnusedSplitLibs:(BOOL)libs;
