//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 1226.2.2.1.0
//
#ifndef HVQueues_h
#define HVQueues_h
@import Foundation;

@class _PASLock;
@protocol OS_dispatch_queue;

@interface HVQueues : NSObject {
  /* instance variables */
  _PASLock *_lock;
  NSObject<OS_dispatch_queue> *_notificationSerialQueue;
}

/* class methods */
+ (id)defaultQueues;

/* instance methods */
- (void)setQueue:(id)queue forDataSource:(unsigned int)source;
- (unsigned char)waitForObserversWithTimeout:(double)timeout;
- (id)init;
- (BOOL)registerQueueObserver:(id)observer dispatchQueue:(id)queue;
- (BOOL)enqueueContent:(id)content contentProtection:(id)protection error:(id *)error;
- (BOOL)dequeueContent:(id *)content contentProtection:(id *)protection dataSource:(unsigned int)source dataSourceContentState:(id)state minimumLevelOfService:(unsigned char)service inMemoryItemsOnly:(BOOL)only error:(id *)error;
- (BOOL)dequeuedContentConsumedWithError:(id *)error;
- (BOOL)dequeuedContentConsumedForDataSources:(unsigned int)sources withError:(id *)error;
- (BOOL)dequeuedContentNotConsumed:(id)consumed dataSource:(unsigned int)source error:(id *)error;
- (BOOL)deleteContentWithRequest:(id)request error:(id *)error;
- (id)statsWithError:(id *)error;
- (BOOL)cleanupWithError:(id *)error shouldContinueBlock:(id /* block */)block;
@end

#endif /* HVQueues_h */
