//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 624.0.100.0.0
//
#ifndef PLSearchResultSection_h
#define PLSearchResultSection_h
@import Foundation;

@class NSArray;

@interface PLSearchResultSection : NSObject

@property (readonly, nonatomic) unsigned long long resultTypes;
@property (readonly, nonatomic) unsigned long long categoryMask;
@property (readonly, nonatomic) NSArray *searchResults;
@property (readonly, nonatomic) NSArray *assetSearchResultsForCompletions;

/* instance methods */
- (id)initWithResultTypes:(unsigned long long)types categoryMask:(unsigned long long)mask searchResults:(id)results assetSearchResultsForCompletions:(id)completions;
@end

#endif /* PLSearchResultSection_h */
