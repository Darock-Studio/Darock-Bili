//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 843.23.0.0.0
//
#ifndef _TtC15SeymourServices39AchievementSessionEvaluationEnvironment_h
#define _TtC15SeymourServices39AchievementSessionEvaluationEnvironment_h
@import Foundation;

@class NSString;

@interface SeymourServices.AchievementSessionEvaluationEnvironment : NSObject { // (Swift)
  /* instance variables */
   achievementDataStore;
   energyBurnedAwardingCache;
   workoutPlanDataStore;
   configurationDataStore;
   session;
   modalityReference;
   planCompletionDate;
   numberOfWorkoutsOver5MinutesForModality;
   numberOfGuidedWorkoutsOver5MinutesForModality;
}

@property (nonatomic) double sessionDuration;
@property (nonatomic, copy) NSString *modalityIdentifier;
@property (nonatomic) BOOL isSessionGuided;
@property (nonatomic) long long numberOfWorkoutsOver5Minutes;
@property (nonatomic) long long currentWeeklyStreak;
@property (nonatomic) long long longestWeeklyStreak;
@property (nonatomic) double sessionEnergyBurned;
@property (nonatomic) double bestEnergyBurnedOfWorkoutModality;
@property (nonatomic) BOOL isSessionCompletingProgram;
@property (nonatomic) BOOL isSessionCompletingPlan;
@property (nonatomic) BOOL isSessionParticipatingInPlan;

/* instance methods */
- (id)valueForUndefinedKey:(id)key;
- (id)init;
@end

#endif /* _TtC15SeymourServices39AchievementSessionEvaluationEnvironment_h */
