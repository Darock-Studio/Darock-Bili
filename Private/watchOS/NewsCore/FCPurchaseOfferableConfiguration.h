//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 617.0.0.0.0
//
#ifndef FCPurchaseOfferableConfiguration_h
#define FCPurchaseOfferableConfiguration_h
@import Foundation;

@class NSString;

@interface FCPurchaseOfferableConfiguration : NSObject

@property (copy, nonatomic) NSString *purchaseID;
@property (nonatomic) BOOL allowsPublisherPhoneApp;
@property (nonatomic) BOOL allowsPublisherPadApp;
@property (nonatomic) BOOL allowsPublisherWebsite;
@property (nonatomic) BOOL preferredOffer;

/* instance methods */
- (id)initWithPurchaseID:(id)id allowsPublisherPhoneApp:(BOOL)app allowsPublisherPadApp:(BOOL)app allowsPublisherWebsite:(BOOL)website preferredOffer:(BOOL)offer;
@end

#endif /* FCPurchaseOfferableConfiguration_h */
