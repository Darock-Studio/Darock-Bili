//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.454, BuildTime: 2024-02-08T22:07:34Z)
//
//    - LC_BUILD_VERSION:  Platform: watchOSSimulator, MinOS: 10.2, SDK: 10.2, Tool: ld (902.8)
//    - LC_SOURCE_VERSION: 1940.42.9.28.8
//
#ifndef GEORPPlaceInfo_h
#define GEORPPlaceInfo_h
@import Foundation;

#include "PBCodable.h"
#include "GEOPDPlaceRequest.h"
#include "GEOPDPlaceResponse.h"
#include "NSCopying-Protocol.h"

@class NSString, PBDataReader;

@interface GEORPPlaceInfo : PBCodable<NSCopying> {
  /* instance variables */
  PBDataReader *_reader;
  unsigned int _readerMarkPos;
  unsigned int _readerMarkLength;
  struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } _readerLock;
  struct { unsigned int x :1 read_placeRequest; unsigned int x :1 read_placeResponse; unsigned int x :1 read_sourceApplication; unsigned int x :1 read_sourceUrl; unsigned int x :1 wrote_anyField; } _flags;
}

@property (readonly, nonatomic) BOOL hasPlaceRequest;
@property (retain, nonatomic) GEOPDPlaceRequest *placeRequest;
@property (readonly, nonatomic) BOOL hasPlaceResponse;
@property (retain, nonatomic) GEOPDPlaceResponse *placeResponse;
@property (readonly, nonatomic) BOOL hasSourceApplication;
@property (retain, nonatomic) NSString *sourceApplication;
@property (readonly, nonatomic) BOOL hasSourceUrl;
@property (retain, nonatomic) NSString *sourceUrl;

/* class methods */
+ (BOOL)isValid:(id)valid;

/* instance methods */
- (void)clearSessionId;
- (void)clearLocations;
- (id)init;
- (id)initWithData:(id)data;
- (id)description;
- (id)dictionaryRepresentation;
- (id)jsonRepresentation;
- (id)initWithDictionary:(id)dictionary;
- (id)initWithJSON:(id)json;
- (void)readAll:(BOOL)all;
- (BOOL)readFrom:(id)from;
- (void)writeTo:(id)to;
- (void)clearSensitiveFields:(unsigned long long)fields;
- (BOOL)hasGreenTeaWithValue:(BOOL)value;
- (void)copyTo:(id)to;
- (id)copyWithZone:(struct _NSZone *)zone;
- (BOOL)isEqual:(id)equal;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
@end

#endif /* GEORPPlaceInfo_h */
