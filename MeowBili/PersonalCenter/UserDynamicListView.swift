...
struct UserDynamicListView: View {
    ...
    func ContinueLoadDynamic() {
        ...
        requestJSON("https://api.bilibili.com/x/polymer/web-dynamic/v1/feed/space?host_mid=\(uid)\({ () -> String in if lastDynamicID != "" { return "&offset=\(lastDynamicID)"; } else { return ""; }; }())", headers: headers) { respJson, isSuccess in
            if isSuccess {
                debugPrint(respJson)
