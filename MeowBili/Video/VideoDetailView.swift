...
struct VideoDetailView: View {
    ...
    var body: some View {
        Group {
            ...
            otherOSMainView
                ...
                .sheet(isPresented: $isMoreMenuPresented) { videoContextActionMenuView }
    ...
    @ViewBuilder var watchMainView: some View {
        #if os(watchOS)
        TabView {
            ZStack {
                Group {
    ...
    @ViewBuilder var otherOSMainView: some View {
        #if !os(watchOS)
        VStack {
            if isDecoded {
            ...
            VStack {
                ...
                .padding(.horizontal)
                .onChange(of: currentDetailSelection) {
                    if isUseExtHaptic {
    ...
    @ViewBuilder var videoContextActionMenuView: some View {
        #if os(watchOS)
        List {
            if videoPages.count <= 1 {
    ...
    #if os(watchOS)
    @ViewBuilder var watchDetailFirstPage: some View {
        VStack {
            Spacer()
            ...
            Spacer()
                .frame(height: 20)
            MarqueeText(text: videoDetails["Title"]!, font: .systemFont(ofSize: 12, weight: .bold), leftFade: 15, rightFade: 15, startDelay: 1.5, alignment: .center)
                .padding(.horizontal, 10)
