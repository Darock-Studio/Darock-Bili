...
struct UserDynamicMainView: View {
                            ...
                            VStack {
                                HStack {
                                    ...
                                    WebImage(url: URL(string: dynamics[i]["SenderPic"]! as! String + "@30w"), options: [.progressiveLoad])
                                        .clipShape(Circle())
                                    #else
                                    AsyncImage(url: URL(string: dynamics[i]["SenderPic"]! as! String + "@30w"))
                                        .clipShape(Circle())
                                    #endif
                                    VStack {
    ...
    func ContinueLoadDynamic() {
        ...
        DarockKit.Network.shared.requestJSON("https://\(UserDefaults.standard.string(forKey: "APIServer") ?? "")/x/polymer/web-dynamic/v1/feed/all?type=all\({ () -> String in if lastDynamicID != "" { return "&offset=\(lastDynamicID)"; } else { return ""; }; }())&page=\(nextLoadPage)", headers: headers) { respJson, isSuccess in
            if isSuccess {
                debugPrint(respJson)
